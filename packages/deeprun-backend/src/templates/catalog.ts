...
export type TemplateValidationProfile = "backend-heavy" | "web-app-light" | "workflow-light";
...
AUTH_TOKEN_SECRET=replace-this-with-a-secure-auth-token-secret-min-32-chars
...
CORS_ALLOWED_ORIGINS=http://localhost:3000
`,
  "Dockerfile": `# deeprun Production Dockerfile
...
const envSchema = z.object({
  ...
  AUTH_TOKEN_SECRET: z.string().min(32).default("replace-this-with-a-secure-auth-token-secret-min-32-chars"),
...
export function getTemplateValidationProfile(templateId: ProjectTemplateId): TemplateValidationProfile {
  ...
  switch (template.type) {
    ...
    default: {
      const unreachable: never = template.type;
